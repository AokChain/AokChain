...
The RPM spec file provided here is for AokChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://aokchain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[AokChain git repository](https://github.com/AokChainNetwork/AokChainNetwork) but are not part of
...
Currently only `Source10` is used, for the example `aokchain.conf` file.
...
in the aokchain git repository. At present only an SVG version of the AokChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the aokchain
...
needed when 0.12.1 is released, a proper fix is already in the AokChain git
master branch.
...
idea to build AokChain Core against a static version of that library compiled
...
The `aokchain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `aokchain-server` package. At this point, most RPM based
...
The `aokchain-server` package should have SELinux support. How to properly do
...
AokChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/aokchain/). He has been
packaging AokChain for Fedora at least since 2012.
...
4. I renamed the `aokchain` package that contains the Qt GUI to `aokchain-core` as
that appears to be how the general population refers to it, in contrast to
`aokchain-xt` or `aokchain-classic`. I wanted to make sure the general population
