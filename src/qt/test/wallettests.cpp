...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<AokChainAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_aokchain-qt -platform xcb      # Linux
//     src/qt/test/test_aokchain-qt -platform windows  # Windows
//     src/qt/test/test_aokchain-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    QString balanceComparison = AokChainUnits::formatWithUnit(unit, balance, false, AokChainUnits::separatorAlways);
    ...
    // Amount input
    AokChainAmountField* amountInput = receiveCoinsDialog.findChild<AokChainAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: aokchain:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_aokchain-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI();
