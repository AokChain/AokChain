...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord *rec, int unit)
{
    ...
    if ((wtx.is_coinbase || wtx.is_coinstake) && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += AokChainUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    if (wtx.is_coinbase || wtx.is_coinstake)
    {
        int maturity = wtx.is_coinbase ? COINBASE_MATURITY : COINSTAKE_MATURITY;
    ...
    for (const CTxOut& txout : wtx.tx->vout) {
        ...
        if (whichType == TX_CLTV)
        {
            ...
            if (nLockTime.getint64() > LOCKTIME_THRESHOLD) {
                strHTML += "<b>" + tr("Lock time") + ":</b> " + QDateTime::fromTime_t(nLockTime.getint64()).toString("yyyy.MM.dd HH:mm") + "<br>";
            } else {
                strHTML += "<b>" + tr("Lock time") + ":</b> Block " + QString::number(nLockTime.getint64()) + "<br>";
            }
        }
    }
