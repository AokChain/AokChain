This directory contains the AokChainQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the AokChain UI.
...
ClientModel is used by the main application `aokchaingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/aokchain/aokchain/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a aokchain:... link from a browser).
...
* UI elements like AokChainAmountField, which inherit from QWidget.
* `aokchainstrings.cpp`: automatically generated
* `aokchainunits.(h/cpp)`: AOK / mAOK / etc handling
* `callback.h`
...
* don't change `local/aokchain_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "aokchain-qt" under "Run", which is where you can also add command line arguments)
