    ...
    for (std::vector<std::pair<CAddressIndexKey, CAmount> >::const_iterator it=addressIndex.begin(); it!=addressIndex.end(); it++) {
        ...
        if (pblockindex->IsProofOfStake()) {
            if (it->first.txindex == 1 && ((chainActive.Height() - it->first.blockHeight) < COINSTAKE_MATURITY))
                immature += it->second; // immature stake output
        } else {
            if (it->first.txindex == 0 && ((chainActive.Height() - it->first.blockHeight) < COINBASE_MATURITY))
                immature += it->second; // immature coinbase output
        }
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The aokchain address to validate"},
                },
                RPCResult{
            "{\n"
    ...
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The aokchain address to use for the signature."},
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "util",               "getaddresstxids",        &getaddresstxids,        {"addresse"} },
    { "util",               "getaddressdeltas",       &getaddressdeltas,       {"addresse"} },
    { "util",               "getaddressbalance",      &getaddressbalance,      {"addresse"} },
    { "util",               "getaddressutxos",        &getaddressutxos,        {"addresse"} },
    { "util",               "getaddressmempool",      &getaddressmempool,      {"addresse"} },
