...
public:
    ...
    UniValue operator()(const CKeyID &keyID) const {
        ...
        obj.pushKV("isofflinestaking", false);
    ...
    UniValue operator()(const std::pair<CKeyID, CKeyID> &keyID) const {
        ...
        CPubKey vchPubKey;
        obj.pushKV("isscript", false);
        obj.pushKV("isofflinestaking", true);
        if (pwallet && pwallet->GetPubKey(keyID.first, vchPubKey)) {
            obj.pushKV("stakingpubkey", HexStr(vchPubKey));
        }
        if(pwallet->GetPubKey(keyID.second, vchPubKey)) {
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"stakingaddress\" : \"address\", (string) The staking address part of a offline staking address\n"
            "  \"spendingaddress\" : \"aaddress\", (string) The spending address part of a offline staking address\n"
    ...
    if (isValid)
    {
        ...
        CAokChainAddress address(request.params[0].get_str());
        if (address.IsOfflineStakingAddress(Params())) {
            CAokChainAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CAokChainAddress spendingAddress;
        ...
        ret.pushKV("isstakable", (mine & ISMINE_STAKABLE || (mine & ISMINE_SPENDABLE &&
                     !address.IsOfflineStakingAddress(Params()))) ? true : false);
