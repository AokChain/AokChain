...
WalletTx MakeWalletTx(interfaces::Chain::Lock& locked_chain, CWallet& wallet, const CWalletTx& wtx)
{
    ...
    for (const auto& txout : wtx.tx->vout) {
        ...
        result.txout_address_is_mine.emplace_back(ExtractDestination(txout.scriptPubKey, result.txout_address.back()) ?
                                                      IsMine(wallet, result.txout_address.back(), chainActive.Tip()) :
                                                      ISMINE_NO);
    }
...
public:
    ...
    bool isSpendable(const CTxDestination& dest) override { return IsMine(*m_wallet, dest, chainActive.Tip()) & ISMINE_SPENDABLE; }
        ...
        std::string* purpose) override
    {
        ...
        if (is_mine) {
            *is_mine = IsMine(*m_wallet, dest, chainActive.Tip());
        }
        if (purpose) {
    ...
    std::vector<WalletAddress> getAddresses() override
    {
        ...
        for (const auto& item : m_wallet->mapAddressBook) {
            result.emplace_back(item.first, IsMine(*m_wallet, item.first, chainActive.Tip()), item.second.name, item.second.purpose);
        }
        return result;
    }
    ...
    unsigned long long getStakeWeight()
    {
        if (!m_wallet)
            return 0;
    ...
    WalletBalances getBalances() override
    {
        ...
        result.locked = m_wallet->GetLockedBalance();
        result.have_watch_only = m_wallet->HaveWatchOnly();
        if (result.have_watch_only) {
            result.watch_only_balance = m_wallet->GetBalance(ISMINE_WATCH_ONLY);
            result.unconfirmed_watch_only_balance = m_wallet->GetUnconfirmedWatchOnlyBalance();
            result.immature_watch_only_balance = m_wallet->GetImmatureWatchOnlyBalance();
            result.watch_stake = m_wallet->GetWatchOnlyStake();
        }
        return result;
    }
