...
IsMineResult IsMineInner(const CKeyStore& keystore, const CScript& scriptPubKey, IsMineSigVersion sigversion, CBlockIndex* bestBlock)
{
    ...
    switch (whichType)
    {
        ...
        case TX_WITNESS_V0_KEYHASH:
        {
            ...
            ret = std::max(ret, IsMineInner(keystore, GetScriptForDestination(CKeyID(uint160(vSolutions[0]))), IsMineSigVersion::WITNESS_V0, bestBlock));
            break;
        }
        case TX_PUBKEYHASH:
        ...
        case TX_SCRIPTHASH:
        {
            ...
            if (keystore.GetCScript(scriptID, subscript)) {
                ret = std::max(ret, IsMineInner(keystore, subscript, IsMineSigVersion::P2SH, bestBlock));
            }
            break;
        }
        case TX_WITNESS_V0_SCRIPTHASH:
        {
            ...
            CScript subscript;
            if (keystore.GetCScript(scriptID, subscript)) {
                ret = std::max(ret, IsMineInner(keystore, subscript, IsMineSigVersion::WITNESS_V0, bestBlock));
            }
            break;
        }
            ...
            if (keystore.HaveKey(keyID))
            {
                ...
                if (nLockTime < LOCKTIME_THRESHOLD) {
                    ...
                    if (nLockTime > bestBlock->nHeight) {
                        return IsMineResult::WATCH_ONLY;
                    } else {
...
{
    switch (IsMineInner(keystore, scriptPubKey, IsMineSigVersion::TOP, bestBlock)) {
...
bool IsTimeLock(const CKeyStore &keystore, const CScript& scriptPubKey, CScriptNum& nLockTime)
{
    ...
    if (whichType == TX_CLTV) {
        CScriptNum lockTime(vSolutions[0], true, 5);
        nLockTime = lockTime.getint64();
        return true;
    }
