...
BOOST_FIXTURE_TEST_SUITE(token_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of AOKCHAINCOIN NOT allowed
        BOOST_CHECK(!IsTokenNameValid("AOK", type));
        BOOST_CHECK(!IsTokenNameValid("AOKCHAIN", type));
        BOOST_CHECK(!IsTokenNameValid("AOKCHAINCOIN", type));

        //- Versions of AOKCHAINCOIN ALLOWED
        BOOST_CHECK(IsTokenNameValid("AOKCHAIN.COIN", type));
        BOOST_CHECK(IsTokenNameValid("AOKCHAIN_COIN", type));
        BOOST_CHECK(IsTokenNameValid("AOKSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDERAOK", type));
        BOOST_CHECK(IsTokenNameValid("AOKCHAINSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDEAOKCHAIN", type));
        BOOST_CHECK(IsTokenNameValid("BLACK_AOKCHAINS", type));
        BOOST_CHECK(IsTokenNameValid("SEAOKOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_token_coin_test)
    {
        ...
        CTokenTransfer token("AOKCHAIN", 1000, 0);
    ...
    BOOST_AUTO_TEST_CASE(new_token_coin_test)
    {
        ...
        // Create the token scriptPubKey
        CNewToken token("AOKCHAIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        token.ConstructTransaction(scriptPubKey);
