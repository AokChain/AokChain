...
{
    // Issuing an Token must contain at least 3 CTxOut( AokChain Burn Tx, Any Number of other Outputs ..., Owner Token Tx, New Token Tx)
    if (vout.size() < 3) {
...
{
    // Must contain at least 3 outpoints (AOK burn, owner change and one or more new unique tokens that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Reissuing an Token must contain at least 3 CTxOut ( AokChain Burn Tx, Any Number of other Outputs ..., Reissue Token Tx, Owner Token Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct AOK Amount
...
{
    // Verify 'AOKq' is in the transaction
...
{
    // Verify 'AOKr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool GetAllMyTokenBalances(std::map<std::string, std::vector<COutput> >& outputs, std::map<std::string, CAmount>& amounts, const std::string& prefix) {
    ...
    return GetAllMyTokenBalancesWallet(vpwallets[0], outputs, amounts, prefix);
}
...
bool CreateTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewToken> tokens, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AokChain address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the AOK required by the burnAmount
    ...
    if (tokenType == KnownTokenType::SUB || tokenType == KnownTokenType::UNIQUE) {
        ...
        for (auto token : tokens) {
            if (!VerifyWalletHasToken(pwallet, parentName + OWNER_TAG, error)) {
                return false;
            }
        }
    }
...
bool CreateReissueTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueToken& reissueToken, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AokChain address: ") + address);
        return false;
    }
    ...
    // Verify that this wallet is the owner for the token, and get the owner token outpoint
    if (!VerifyWalletHasToken(pwallet, token_name + OWNER_TAG, error)) {
        return false;
    }
...
bool CreateTransferTokenTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CTokenTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any AOK, transfering an token requires a network fee"));
        return false;
    }
    ...
    for (auto transfer : vTransfers) {
        ...
        if (!VerifyWalletHasToken(pwallet, token_name, error)) // Sets error if it fails
            return false;
