...
#ifndef AOKCHAIN_WALLET_WALLET_H
#define AOKCHAIN_WALLET_WALLET_H
...
public:
    ...
    CAmount GetImmatureStakeCredit(bool fUseCache=true) const EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    CAmount GetAvailableStakableCredit() const NO_THREAD_SAFETY_ANALYSIS;
    CAmount GetLockedCredit(bool fUseCache=true) const NO_THREAD_SAFETY_ANALYSIS;
    CAmount GetImmatureWatchOnlyCredit(const bool& fUseCache=true) const NO_THREAD_SAFETY_ANALYSIS;
    CAmount GetAvailableWatchOnlyCredit(const bool& fUseCache=true) const NO_THREAD_SAFETY_ANALYSIS;
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapTokenCoins,
                            bool fGetAOK = true, bool fOnlyTokens = false,
                            ...
                            const int& nMinDepth = 0, const int& nMaxDepth = 9999999, bool fLockedTokens = false) const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);
    ...
    void AvailableTokens(std::map<std::string, std::vector<COutput> > &mapTokenCoins, bool fOnlySafe = true,
                         ...
                         const uint64_t &nMaximumCount = 0, const int &nMinDepth = 0, const int &nMaxDepth = 9999999) const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Tokens and AOK
     */
    ...
    bool SelectCoinsForStaking(CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);

    /**
     * populate vCoins with vector of available COutputs.
     */
    void AvailableCoinsForStaking(std::vector<COutput>& vCoins) const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlySafe=true, const CCoinControl *coinControl = nullptr,
                        const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY,
                        const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t& nMaximumCount = 0,
                        const int& nMinDepth = 0, const int& nMaxDepth = 9999999) const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);

    uint64_t GetStakeWeight() const;
    bool HaveAvailableCoinsForStaking() const EXCLUSIVE_LOCKS_REQUIRED(cs_main, cs_wallet);
    ...
    /** TOKENS START */
    bool CreateTransactionWithTokens(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string message, int& nChangePosInOut,
    ...
    bool CreateTransactionWithTransferToken(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string message, int& nChangePosInOut,
                                                     std::string& strFailReason, const CCoinControl& coin_control, bool sign = true);

    bool CreateTransactionWithReissueToken(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string message, int& nChangePosInOut,
    ...
    bool CreateTransactionAll(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string message, int& nChangePosInOut,
    ...
    bool CreateTransactionAll(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string message,
...
#endif // AOKCHAIN_WALLET_WALLET_H
