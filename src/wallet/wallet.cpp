...
void CWallet::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex *pindex, const std::vector<CTransactionRef>& vtxConflicted) {
    ...
    for (const CTransactionRef& ptx : vtxConflicted) {
        ...
        SyncTransaction(ptx, {} /* block hash */, posInBlock);
...
bool CWallet::IsChange(const CScript& script) const
{
    ...
    if (::IsMine(*this, script, chainActive.Tip()))
    {
        ...
        while (block_height && !fAbortRescan && !ShutdownRequested()) {
            ...
            if (chain().findBlock(block_hash, &block) && !block.IsNull()) {
                ...
                if (!locked_chain->getBlockHeight(block_hash)) {
                    ...
                    // https://github.com/aokchain/aokchain/pull/14711#issuecomment-458342518
...
CAmount CWalletTx::GetCredit(interfaces::Chain::Lock& locked_chain, const isminefilter& filter) const
{
    ...
    if (IsImmatureCoin(locked_chain))
        return 0;
...
{
    if (IsImmatureCoin(locked_chain) && IsInMainChain(locked_chain)) {
...
CAmount CWalletTx::GetImmatureStakeCredit(interfaces::Chain::Lock& locked_chain, bool fUseCache) const
{
    if (IsImmatureCoin(locked_chain))
    {
...
CAmount CWalletTx::GetLockedCredit(interfaces::Chain::Lock& locked_chain, bool fUseCache) const
{
    if (pwallet == nullptr)
        return 0;
    ...
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) >= min_depth) {
                nTotal += pcoin->GetAvailableCredit(*locked_chain, false, filter);
            }
        }
    }
    ...
    CAmount nTotal = 0;
    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted(*locked_chain))
                nTotal += pcoin->GetLockedCredit(*locked_chain, false);
        }
    }
    ...
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (!pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) == 0 && pcoin->InMempool())
                nTotal += pcoin->GetAvailableCredit(*locked_chain, false);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetImmatureBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += pcoin->GetImmatureCredit(*locked_chain, false);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetStakeBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += pcoin->GetImmatureStakeCredit(*locked_chain);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetUnconfirmedWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            const CWalletTx* pcoin = &entry.second;
            if (!pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) == 0 && pcoin->InMempool())
                nTotal += pcoin->GetAvailableCredit(*locked_chain, false, ISMINE_WATCH_ONLY);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetImmatureWatchOnlyBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &entry.second;
            nTotal += pcoin->GetImmatureWatchOnlyCredit(*locked_chain, false);
        }
    }
    return nTotal;
}
...
CAmount CWallet::GetLegacyBalance(const isminefilter& filter, int minDepth) const
{
    ...
    for (const auto& entry : mapWallet) {
        ...
        if (depth < 0 || !CheckFinalTx(*wtx.tx) || wtx.IsImmatureCoin(*locked_chain)) {
            continue;
        }
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth) const
{
    ...
    for (const auto& entry : mapWallet)
    {
        ...
        if (pcoin->IsImmatureCoin(locked_chain))
            continue;
...
{
    {
        ...
        if (fBroadcastTransactions)
        {
            ...
            if (!wtx.AcceptToMemoryPool(*locked_chain, maxTxFee, state)) {
                ...
                AbandonTransaction(*locked_chain, wtxNew.tx->GetHash());
                return false;
            } else {
...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    {
        if (pcoin.first->GetDepthInMainChain(*locked_chain) >= COINSTAKE_MATURITY) {
...
int CMerkleTx::GetBlocksToMaturity(interfaces::Chain::Lock& locked_chain) const
{
    ...
    int maturity = IsCoinBase() ? COINBASE_MATURITY : COINSTAKE_MATURITY;
    int chain_depth = GetDepthInMainChain(locked_chain);
    assert(chain_depth >= 0); // coinbase tx should not be conflicted
    return std::max(0, (maturity+1) - chain_depth);
}

bool CMerkleTx::IsImmatureCoin(interfaces::Chain::Lock& locked_chain) const
{
