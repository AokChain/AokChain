...
CWallet *GetWalletForJSONRPCRequest(const JSONRPCRequest& request)
{
    ...
    if (GetWalletNameFromJSONRPCRequest(request, wallet_name)) {
        CWallet* pwallet = GetWallet(wallet_name);
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new AokChain address for receiving payments.\n"
            "If 'label' is specified, it is added to the address book \n"
            "so payments received with the address will be associated with 'label'.\n"
            "\nArguments:\n"
            "1. \"label\"          (string, optional) The label name for the address to be linked to. If not provided, the default label \"\" is used. It can also be set to the empty string \"\" to represent the default label. The label does not need to exist, it will be created if there is no label by the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new aokchain address\n"
            "\nExamples:\n"
...
UniValue getofflinestakingaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("getofflinestakingaddress", "\"KgZDBz1dV8H4zcZUZ3yV1336bKkQ5L4FXv\" \"Ktf4i17uoofsKJqMTuLsqGBzHY92bfjPux\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getofflinestakingaddress", "\"KgZDBz1dV8H4zcZUZ3yV1336bKkQ5L4FXv\", \"Ktf4i17uoofsKJqMTuLsqGBzHY92bfjPux\"")
        );
    ...
    CAokChainAddress stakingAddress(request.params[0].get_str());
    CKeyID stakingKeyID;
    ...
    CAokChainAddress spendingAddress(request.params[1].get_str());
    ...
    return CAokChainAddress(stakingKeyID, spendingKeyID).ToString();
}
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 0)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new AokChain address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (IsMine(*pwallet, dest, chainActive.Tip())) {
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The aokchain address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a aokchain address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, const CCoinControl& coin_control, int64_t lockTime = 0, std::string message = "")
{
    ...
    // Parse AokChain address
    CScript scriptPubKey = GetScriptForDestination(address, lockTime);
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "                             The recipient will receive less aokchains than you enter in the amount field.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The aokchain address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The aokchain address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The aokchain address for transactions.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        for (const CTxOut& txout : wtx.tx->vout)
        {
            ...
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*pwallet, address, chainActive.Tip()) && setAddress.count(address)) {
                if (wtx.GetDepthInMainChain() >= nMinDepth)
                    nAmount += txout.nValue;
            }
        }
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a aokchain address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The aokchain address to send funds to.\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"label\" )\n"
            ...
            "Each key is a AokChain address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of aokchain addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) aokchain address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"        (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A aokchain address associated with the keys.\n"
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool by_label) EXCLUSIVE_LOCKS_REQUIRED(cs_main, pwallet->cs_wallet)
{
    // Minimum confirmations
    int nMinDepth = 1;
    if (!params[0].isNull())
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        for (const CTxOut& txout : wtx.tx->vout)
        {
            ...
            isminefilter mine = IsMine(*pwallet, address, chainActive.Tip());
            if(!(mine & filter))
                continue;
 ...
 */
void ListTransactions(CWallet* const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, UniValue& retTokens, const isminefilter& filter) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    CAmount nFee;
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        for (const COutputEntry& s : listSent)
        {
            ...
            if (involvesWatchonly || (::IsMine(*pwallet, s.destination, chainActive.Tip()) & ISMINE_WATCH_ONLY)) {
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        for (const COutputEntry& r : listReceived)
        {
            ...
            {
                UniValue entry(UniValue::VOBJ);
                if (involvesWatchonly || (::IsMine(*pwallet, r.destination, chainActive.Tip()) & ISMINE_WATCH_ONLY)) {
                    entry.pushKV("involvesWatchonly", true);
                }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The aokchain address of the transaction. Not present for \n"
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    ...
    for (const std::pair<CTxDestination, CAddressBookData>& entry : pwallet->mapAddressBook) {
        if (IsMine(*pwallet, entry.first, chainActive.Tip()) & includeWatchonly) {  // This address belongs to me
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The aokchain address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The aokchain address involved in the transaction\n"
    ...
    entry.pushKV("token_details", tokenDetails);
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (pwallet->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3)) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending aokchains\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending aokchain\n"
    ...
    StartShutdown();
    return "wallet encrypted; AokChain server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending aokchains.\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    obj.pushKV("offline_staking_balance", ValueFromAmount(pwallet->GetOfflineStakingBalance()));
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of aokchain addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the aokchain address\n"
...
UniValue listunspenttoken(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() < 1 && request.params.size() > 6))
        throw std::runtime_error(
            ...
            "4. \"addresses\"      (string) A json array of aokchain addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) aokchain address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "\nExamples\n"
            + HelpExampleCli("listunspenttoken", "")
            + HelpExampleCli("listunspenttoken", "EXAMPLE 6 9999999 \"[\\\"1PGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"1LtvqCaApEdUGFkpKMM4MstjcaL4dKg8SP\\\"]\"")
            + HelpExampleRpc("listunspenttoken", "EXAMPLE 6, 9999999 \"[\\\"1PGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"1LtvqCaApEdUGFkpKMM4MstjcaL4dKg8SP\\\"]\"")
            + HelpExampleCli("listunspenttoken", "EXAMPLE 6 9999999 '[]' true '{ \"minimumAmount\": 0.005 }'")
            + HelpExampleRpc("listunspenttoken", "EXAMPLE 6, 9999999, [] , true, { \"minimumAmount\": 0.005 } ")
        );
    ...
    pwallet->AvailableTokens(mapTokens, !include_unsafe, nullptr, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth);
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The aokchain address to receive the change\n"
...
//                {"replaceable", UniValueType(UniValue::VBOOL)},
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid aokchain address");
            }
...
{
    throw std::runtime_error("bumpfee has been deprecated on the AOK Wallet.");
...
{ //  category              name                        actor (function)           argNames
    ...
    { "wallet",             "listunspenttoken",         &listunspenttoken,         {"token_name", "minconf","maxconf","addresses","include_unsafe","query_options"} },
