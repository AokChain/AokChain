 ...
 */
static const char* AOKCHAIN_PID_FILENAME = "aokchaind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", AOKCHAIN_PID_FILENAME)));
}
 ...
 * between aokchaind, and aokchain-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", AOKCHAIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", AOKCHAIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-staking", "Enable staking", false, OptionsCategory::BLOCK_CREATION);
...
/** Sanity checks
 *  Ensure that AokChain is running in a usable environment with all
...
{
    // Make sure only a single AokChain process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", AOKCHAIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if aokchain is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if aokchain is started while in a temporary directory.\n",
